...
# Path of qar_dictionary.txt - fmdl_dictionary.txt - mtar_dictionary.txt - skeleton_hash.txt - presets.xml
DICTIONARY_FOLDER = r"C:\Users\Ali\Desktop\MGS Vtpp blender\444\dictionary"

# Path of CityHash.dll - System.Half.dll - FMSv2Portable.dll
DLL_FOLDER = r"C:\Users\Ali\Desktop\MGS Vtpp blender\444\references"  
...
#---------------------------------------------------------------------------------
    ...
    def __init__(self, filepath, dict_manager):
        ...
        self.bone_defs()                    # feature 0
        self.mesh_group_headers()           # feature 1
        self.mesh_group_defs()              # feature 2
        self.mesh_defs()                    # feature 3
        self.material_instance_headers()    # feature 4
        self.bone_groups()                  # feature 5
        self.texture_refs()                 # feature 6
        self.material_parameters()          # feature 7
        self.shader_aliases()               # feature 8
        self.mesh_data_layouts()            # feature 9
        self.mesh_buffer_headers()          # feature 10
        self.mesh_buffer_format_elements()  # feature 11
        self.aabbs()                        # feature 12
        self.file_mesh_buffer_headers()     # feature 13
        self.lod_Info()                     # feature 14
        self.ibuffer_slices()               # feature 15
        self.type18()                       # feature 16
        self.type20()                       # feature 17
        self.paths()                        # feature 18
        self.names()                        # feature 19
...
#---------------------------------------------------------------------------------
    ...
    def _read_material_types(self):
        ...
        if not fh:
            logger.warning("No MATERIALS feature (8)")
            self.material_types = []
            return
        ...
        logger.start(f"Reading {count} material types @ 0x{offset:08X}")
        self.material_types = []
        
        for i in range(count):
            type_offset = offset + (i * 8)  # 8 bytes per entry
            if type_offset + 8 > len(self.data):
                break
            ...
            if i < 5:
                logger.info(f"MatType {i}: {shader_name}")
        
        logger.success("Material types parsed", f"{len(self.material_types)} types")

#---------------------------------------------------------------------------------

    # --- 4.??: Material Instances ---
    def read_materials(self):
        """Parse MATERIAL_INSTANCES (feature 4)"""
        logger.sub_section("Reading Material Instances")
        fh = self._get_feature_header(4)
        
        if not fh:
            logger.warning("No MATERIAL_INSTANCES feature (4)")
            return 0
        
        # ✅ اضافه شده: خواندن material types اول
        if not hasattr(self, 'material_types'):
            self._read_material_types()
        
        offset = self.header['features_data_offset'] + fh['data_offset']
        count = fh['total_count']
        mat_size = 0x10  # 16 bytes
        ...
        for i in range(count):
            ...
            if i < 5:
                logger.info(f"Mat{i:2d}: {mat_name:<25} shader={shader_name}")
        
        logger.success("Materials parsed", f"{len(self.materials)} materials")
        return len(self.materials)
    
    def _resolve_material_name(self, name_index):
        """Resolve material name from StrCode64"""
        if name_index < len(self.names):
            hash_val = self.names[name_index].get('hash')
            if hash_val and self.dict:
                name = self.dict.get_bone_name(hash_val)  # uses same hash lookup
                if name and not name.startswith("Bone_") and not name.startswith("0x"):
                    return name
            return f"Mat_{name_index:03d}_{hash_val:08X}" if hash_val else f"Mat_{name_index:03d}"
    ...
    def _get_texture_role(self, tex_param_idx):
        ...
        fh = self._get_feature_header(7)
        if not fh:
            return 'Base_Tex_SRGB'
        
        offset = self.header['features_data_offset'] + fh['data_offset']
        param_offset = offset + (tex_param_idx * 4)
        ...
        # Map to known roles
        role_map = {
            'Base_Tex_SRGB': 'Base_Tex_SRGB',
            'NormalMap_Tex_NRM': 'NormalMap_Tex_NRM',
            'SpecularMap_Tex_LIN': 'SpecularMap_Tex_LIN',
            'Translucent_Tex_LIN': 'Translucent_Tex_LIN',
        }
        
        for key in role_map:
            if key.lower() in role.lower():
                return role_map[key]
        return role if 'Tex_' in role else 'Base_Tex_SRGB'
...
#--------------------------------------------------------------------------------- 
    ...
    # --- 4.??: Material Parameter Vectors (Section 1 Block 0) ---
...
#---------------------------------------------------------------------------------
    ...
    # --- 4.??: Vertex Buffer Reader (کاملاً اصلاح شده) ---
...
#---------------------------------------------------------------------------------
    ...
    # --- 4.2.1: Complete Feature Type Names ---
...
#---------------------------------------------------------------------------------
    ...
    # --- 4.4: Bone Defs (FmdlBone struct from Fmdl.cs) ---
    def read_bone_defs(self):
        ...
        for i in range(count):
            ...
            # nameIndex (2), parentIndex (2), boundingBoxIndex (2), Flags(has Bounding Box , B : 1 , C : 1) (2), padding (8)
...
#---------------------------------------------------------------------------------
    ...
    # --- 4.7: Mesh Defs (FmdlMeshInfo struct) ---
    def read_mesh_defs(self):
        ...
        for i in range(count):
            ...
            # materialInstanceIndex (2), boneGroupIndex (2), meshIndex (2), vertexCount (2)
            # dataLayoutIndex (2), firstFaceVertexIndex (4), faceVertexCount (4)
            # firstFaceInfoIndex (4), unknown2 (4), padding (8)
            ...
            # ✅ اضافه شده: data_layout_index (مهم!)
            data_layout_idx = struct.unpack('<H', self.data[mesh_offset+12:mesh_offset+14])[0]
            ...
            first_face_info = struct.unpack('<I', self.data[mesh_offset+24:mesh_offset+28])[0]
            ...
            self.meshes.append({
                ...
                'data_layout_index': data_layout_idx,  # ✅ اضافه شد
                'vertices_start_index': first_face_vert,
            ...
            if i < 5:
                logger.info(f"Mesh {i:2d}: verts={vert_count:4d}, faces={face_vert_count//3:4d}, "
                           f"mat={material_idx}, boneGroup={bone_group_idx}, layout={data_layout_idx}")
...
#---------------------------------------------------------------------------------
    ...
    # --- 4.9: Bone Groups (FmdlBoneGroup struct) ---
...
#---------------------------------------------------------------------------------
    ...
    def read_texture_refs(self):
        """Parse TEXTURES (feature 6)"""
        logger.sub_section("Reading Texture References")
        fh = self._get_feature_header(6)
        ...
        for i in range(count):
            ...
            if i < 10:
                logger.info(f"Tex{i:2d}: {tex_name:<30} path={tex_path[:30] if tex_path else 'None':<30}")
    ...
    def _resolve_texture_name(self, name_index):
        """Resolve texture name from StrCode64"""
        if name_index < len(self.names):
            hash_val = self.names[name_index].get('hash')
            if hash_val:
                # Try dictionary
                if self.dict:
                    ...
                    if name and not name.startswith("Bone_") and not name.startswith("0x"):
                        return name
                # Return hash-based name
                return f"tex_{hash_val:016X}"
        return f"Tex_{name_index:03d}"
    
    # --- 4.10.2: Texture Path ---
    def _resolve_texture_path(self, path_index):
        """Resolve texture path from PathCode64"""
        if path_index < len(self.paths):
            hash_val = self.paths[path_index].get('hash')
            if hash_val:
                # Try QAR dictionary
                if self.dict:
                    path = self.dict.get_texture_path(hash_val)
                    if path:
                        return path
                return f"/assets/tpp/ftex/{hash_val:016X}"
        return None
        
#---------------------------------------------------------------------------------

    # --- 4.11: Material Parameter ---
...
#---------------------------------------------------------------------------------
    ...
    # --- 4.13: Mesh Data Layout Descriptions (Critical for Vertex Format) ---
...
#---------------------------------------------------------------------------------
    ...
    # --- 4.14: Mesh Buffer Headers (Vertex Stream Headers - Critical!) ---
...
#---------------------------------------------------------------------------------
    ...
    # --- 4.15: Mesh Buffer Format Elements (Vertex Attributes - Critical) ---
...
#---------------------------------------------------------------------------------
    ...
    # --- 4.16: AABB Reader (FmdlBoundingBox struct) ---
    def read_aabbs(self):
...
#---------------------------------------------------------------------------------
    ...
    # --- 4.22: Paths (PathCode64) - Texture/Material paths ---
    def read_paths(self):
    ...
    # --- 4.5.1: Feature Header Lookup (Optimized) ---
...
#---------------------------------------------------------------------------------  
    ...
    # --- 8.4: Mesh Builder - Material Assignment ---
...
def parse_fmdl(filepath, dict_folder):
    ...
    parser.read_mesh_group_headers()            # feature 1
    parser.read_mesh_group_defs()               # feature 2
    parser.read_mesh_defs()                     # feature 3
    parser.read_material_instance_headers()     # feature 4
    parser.read_bone_groups()                   # feature 5
    parser.read_texture_refs()                  # feature 6
    parser.read_material_parameters()           # feature 7
    parser.read_shader_aliases()                # feature 8
    ...
    parser.read_aabbs()                         # feature 12
    parser.read_file_mesh_buffer_headers()      # feature 13
    parser.read_lod_Info()                      # feature 14
    parser.read_ibuffer_slices()                # feature 15
    parser.read_type_18()                       # feature 16
    parser.read_type_20()                       # feature 17
    parser.read_paths()                         # feature 18
    parser.read_names()                         # feature 19
    ...
    result = {
        ...
        'bone_defs': parser.bone_defs,
        'mesh_group_headers': parser.mesh_group_headers,
        'mesh_group_defs': parser.mesh_group_defs,
        'mesh_defs': parser.mesh_defs,
        'material_instance_headers': parser.material_instance_headers,
        'bone_groups': getattr(parser, 'bone_groups', []), 
        'texture_refs':pareser.texture_ref,
        'materials_parameters': parser.materials_parameters,
        'texture_refs': parser.texture_refs,
        'shader_aliases':parser.shader_aliases,
        ...
        'lod_info': parser.lod_info,
        'ibuffer_slices': parser.ibuffer_slices,
        'type_18': parser.type_18,
        'type_20': parser.type_20,
        'paths': parser.paths,
        'names': parser.names,
    }

    logger.section("PARSING COMPLETE")
    logger.result("Bones", len(result['bone_defs']))
    logger.result("Bone Groups", len(result['bone_groups']))
    logger.result("Meshes", len(result['mesh_defs']))
    logger.result("Materials", len(result['materials_parameters']))
